...
def purchase_confirmation_sending_buyer(self, listing):
    text = PURCHASE_ITEM_BUYER_TEXT % (self.user.username, listing.title,
                                       "%.02f" % float(listing.selling_price),
    ...
    if address:
        ...
        if address.address_line2:
            address_row = '%s %s, %s %s' % (address.address_line1, address.address_line2, address.postcode,
                                            address.city)
        text = PURCHASE_ITEM_SELLER_TEXT_WITH_ADDRESS % (listing.user.username,
                                                         listing.title, self.user.username,
                                                         "%.02f" % float(listing.selling_price),
                                                         self.user.email, address_row, self.user.username)
    else:
        text = PURCHASE_ITEM_SELLER_TEXT % (listing.user.username, listing.title,
                                            self.user.username,
                                            "%.02f" % float(listing.selling_price),
...
def listing_with_issue_seller(self, listing):
    text = LISTING_WITH_ISSUE_SELLER_TEXT % (listing.user.username, listing.title,
                                             self.user.username)
...
def listing_with_issue_investigation_opened_buyer(order):
    text = LISTING_WITH_ISSUE_INVESTIGATION_OPENED_TEXT % (order.user.username,
                                                           order.listing.title)
...
def send_warning_4_6_days_email(email, username, title, number_of_days):
    text = HAS_ITEM_RECEIVED_TEXT % (username, number_of_days, title)
